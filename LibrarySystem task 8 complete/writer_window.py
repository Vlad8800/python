...
def show_books_by_author(author):
    ...
    try:
        ...
        books_window.geometry("800x500")

        Label(books_window, text=f"Книги автора: {name} {surname}", font=("Arial", 14)).pack(pady=10)

        tree = ttk.Treeview(books_window, columns=("ID", "Назва", "Рік", "Мова", "Інвентарний номер", "Категорія"), show="headings")
        ...
        tree.column("Назва", width=200)
        tree.column("Рік", width=80)
        tree.column("Мова", width=100)
        tree.column("Інвентарний номер", width=150)
        tree.column("Категорія", width=150)
        ...
        button_frame = Frame(books_window)
        button_frame.pack(pady=10)
        
        Button(button_frame, text="Редагувати обрану книгу", 
               command=lambda: edit_selected_book(tree, author)).pack(side=LEFT, padx=5)
        
        Button(button_frame, text="Видалити обрану книгу", 
               command=lambda: delete_selected_book(tree, author)).pack(side=LEFT, padx=5)
...
def delete_selected_book(tree, author):
    selected = tree.selection()
    if not selected:
    ...
    book_id = tree.item(selected[0])['values'][0]
    title = tree.item(selected[0])['values'][1]
    
    if messagebox.askyesno("Підтвердження", f"Ви впевнені, що хочете видалити книгу '{title}'?"):
        delete_book(author, book_id)
        # Оновлюємо список книг
        show_books_by_author(author)

def delete_book(author, book_id):
    user_id = get_user_id(author)
    if user_id is None:
        messagebox.showerror("Помилка", "Автор не знайдений")
        return
    ...
    try:
        ...
        # Перевіряємо, чи книга належить авторові
        cursor.execute("""
            SELECT b.book_id FROM Books b
            JOIN Authors a ON b.author_id = a.author_id
            WHERE a.user_id = %s AND b.book_id = %s
        """, (user_id, book_id))
        
        if not cursor.fetchone():
            messagebox.showerror("Помилка", "Ця книга не належить вам")
            return
...
def show_edit_book_window(author, book_id):
    def save_changes():
        ...
        try:
            ...
            cursor.execute("""
                ...
                WHERE book_id = %s
            """, (title, category_id, year, language, book_id))
    ...
    Label(edit_window, text="Редагування книги", font=("Arial", 14)).pack(pady=10)

    Label(edit_window, text="Назва:").pack(pady=5)
    entry_title = Entry(edit_window, width=40)
    entry_title.insert(0, title)
    entry_title.pack(pady=5)

    Label(edit_window, text="Рік:").pack(pady=5)
    entry_year = Entry(edit_window, width=40)
    entry_year.insert(0, year)
    entry_year.pack(pady=5)

    Label(edit_window, text="Мова:").pack(pady=5)
    entry_language = Entry(edit_window, width=40)
    entry_language.insert(0, language)
    entry_language.pack(pady=5)

    Label(edit_window, text="Категорія:").pack(pady=5)
    ...
    Button(edit_window, text="Зберегти зміни", command=save_changes).pack(pady=20)
    Button(edit_window, text="Скасувати", command=edit_window.destroy).pack(pady=5)
    def save_changes():
        title = entry_title.get().strip()
        year = entry_year.get().strip()
        language = entry_language.get().strip()
    ...
    try:
        ...
        # Отримуємо назву категорії
        cursor.execute("SELECT name FROM Categories WHERE category_id = %s", (category_id,))
        category_name = cursor.fetchone()[0] if cursor.fetchone() else ""
...
def generate_collection_inventory():
    """Генерація унікального інвентарного номера для збірки"""
    conn = None
    cursor = None
    try:
        conn = get_db_connection()
        ...
        print(f"Помилка генерації номера: {e}")
        return "COLL-00001"
    finally:
        if cursor:
            cursor.close()
...
def show_writer_window(user_login):
    ...
    writer_window.geometry("400x350")  # Збільшимо висоту для нової кнопки
    ...
    Button(menu_frame, text="Переглянути мої книги", 
           command=lambda: show_books_by_author(user_login)).pack(pady=5)
    Button(menu_frame, text="Додати нову книгу", 
           command=lambda: show_add_book_window(user_login)).pack(pady=5)
    Button(menu_frame, text="Створити збірку", 
           command=show_collection_window).pack(pady=5)
    Button(menu_frame, text="Редагувати мої книги", 
           command=lambda: show_books_by_author(user_login)).pack(pady=5)  # Нова кнопка
    Button(menu_frame, text="Вийти", 
           command=writer_window.destroy).pack(pady=20)
