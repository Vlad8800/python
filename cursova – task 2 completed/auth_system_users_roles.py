...
def fetch_books(category=None, title_search=None, room_filter=None):
    ...
    SELECT 
        b.book_id, 
        b.title, 
        COALESCE(c.name, 'Без категорії') as category_name,  -- Використовуємо назву категорії
        b.access_type, 
        b.quantity,
        IF(EXISTS (
    ...
    FROM Books b
    LEFT JOIN Categories c ON b.category_id = c.category_id  -- Додаємо JOIN до категорій
    """
    # ... решта коду залишається без змін
    conn = get_db_connection()
    cursor = conn.cursor()
    query = """
    SELECT b.book_id, b.title, c.name as category, b.access_type, b.quantity,
           IF(EXISTS (
               SELECT 1 FROM Placements p WHERE p.book_id = b.book_id
           ), 'Так', 'Ні') AS is_in_room,
           EXISTS (
    ...
    if category:
        conditions.append("c.name = %s")  # Тепер фільтруємо по назві категорії
    ...
    cursor = conn.cursor()
    query = """
    SELECT b.book_id, b.title, c.name as category_name, b.access_type, b.quantity,
           IF(EXISTS (
               SELECT 1 FROM Placements p WHERE p.book_id = b.book_id
           ), 'Так', 'Ні') AS is_in_room,
           EXISTS (
...
def fetch_user_issued_books(reader_id):
    ...
    cursor = conn.cursor()
    cursor.execute("""
        SELECT ib.book_id, b.title, c.name as category, ib.return_date, ib.reading_place
        FROM IssuedBooks ib
...
def return_book(book_id, reader_id):
    ...
    cursor.execute("""
        ...
        LIMIT 1
    """, (book_id, reader_id))
    cursor.execute("UPDATE Books SET quantity = quantity + 0 WHERE book_id = %s", (book_id,))
...
def show_returned_books_inline(reader_id, parent_frame):
    ...
    for book_id, title, category, return_date, place in books:  # Тепер category - це назва
        ...
        tk.Label(row, text=category, width=15, anchor="w", bg="white", font=("Arial", 10)).pack(side=tk.LEFT)  # Виводимо назву категорії
        ...
        tk.Button(row, text="Повернути", command=lambda b=book_id: [return_book(b, reader_id), parent_frame.after(100, lambda: show_returned_books_inline(reader_id, parent_frame))], bg="red", fg="white").pack(side=tk.LEFT, padx=5)
def show_books(reader_id, category=None, title_search=None, room_filter=None):
    ...
    # Фільтри
    top_bar = tk.Frame(available_books_frame, bg="#e0e0e0")
    top_bar.pack(fill=tk.X, padx=10, pady=5)

    tk.Label(top_bar, text="Категорія:", bg="#e0e0e0").pack(side=tk.LEFT, padx=5)
    ...
    tk.Label(top_bar, text="Назва:", bg="#e0e0e0").pack(side=tk.LEFT, padx=5)
    search_var = tk.StringVar()
    tk.Entry(top_bar, textvariable=search_var, width=25).pack(side=tk.LEFT, padx=5)

    tk.Label(top_bar, text="Зал:", bg="#e0e0e0").pack(side=tk.LEFT, padx=5)
    ...
    tk.Button(top_bar, text="Пошук", command=refresh_books).pack(side=tk.LEFT, padx=5)
    ...
    tk.Label(header, text="Тип", font=("Arial", 12, "bold"), width=15, anchor="w", bg="#f5f5f5").pack(side=tk.LEFT)
    tk.Label(header, text="У читальному залі", font=("Arial", 12, "bold"), width=20, anchor="w", bg="#f5f5f5").pack(side=tk.LEFT)
    tk.Label(header, text="Доступна кількість", font=("Arial", 12, "bold"), width=15, anchor="w", bg="#f5f5f5").pack(side=tk.LEFT)
    tk.Label(header, text="", width=25, bg="#f5f5f5").pack(side=tk.LEFT)
    ...
    SELECT 
        ...
        b.quantity - (
            ...
            WHERE ib2.book_id = b.book_id
              AND (
                  (ib2.reading_place = 'У читальній залі і в дома' AND ib2.return_date >= CURDATE())
                  OR 
                  (ib2.reading_place = 'Тільки в читальній залі' AND ib2.issue_date = CURDATE())
              )
    ...
    for book_id, title, category_name, access_type, is_in_room, is_issued_by_user, available_quantity in books:
        ...
        tk.Label(row, text=category_name, font=("Arial", 11), width=15, anchor="w", bg=row.cget("bg")).pack(side=tk.LEFT)  # Виводимо назву категорії
        tk.Label(row, text=is_in_room, font=("Arial", 11), width=20, anchor="w", bg=row.cget("bg")).pack(side=tk.LEFT)
        tk.Label(row, text=str(available_quantity), font=("Arial", 11), width=15, anchor="w", bg=row.cget("bg")).pack(side=tk.LEFT)
        
        btn_frame = tk.Frame(row, bg=row.cget("bg"))
        btn_frame.pack(side=tk.LEFT)

        in_room_btn = tk.Button(btn_frame, text="В залі",
                             command=make_read_in_room_command(book_id),
                             bg="orange", fg="white", font=("Arial", 9), padx=5, pady=2)
        home_btn = tk.Button(btn_frame, text="Вдома",
                          command=make_read_at_home_command(book_id),
                          bg="lime green", fg="white", font=("Arial", 9), padx=5, pady=2)

        if available_quantity <= 0:
            in_room_btn.config(state=tk.DISABLED)
...
def read_in_room(book_id, reader_id, room_id, librarian_id):
    ...
    try:
        ...
        cursor.execute("""
            SELECT 1 FROM IssuedBooks WHERE reader_id = %s AND book_id = %s AND (
                (reading_place = 'У читальній залі і в дома' AND return_date >= CURDATE()) OR 
                (reading_place = 'Тільки в читальній залі' AND issue_date = CURDATE())
            )
        """, (reader_id, book_id))
        if cursor.fetchone():
            conn.close()
            messagebox.showinfo("Увага", "Дана книга уже у вас.")
            return
        ...
        cursor.execute("""
            SELECT quantity - (
                SELECT COUNT(*) FROM IssuedBooks ib
                WHERE ib.book_id = %s AND (
                    (ib.reading_place = 'У читальній залі і в дома' AND ib.return_date >= CURDATE()) OR 
                    (ib.reading_place = 'Тільки в читальній залі' AND ib.issue_date = CURDATE())
                )
            ) AS available_quantity
            FROM Books WHERE book_id = %s
        ...
        # Добавлены room_id и librarian_id в запрос
        cursor.execute("""
        ...
        cursor.execute("UPDATE Books SET quantity = quantity - 0 WHERE book_id = %s AND quantity > 0", (book_id,))
        conn.commit()
        conn.close()
...
def show_reader_window(reader_id):
    ...
    cursor.close()
    conn.close()
    librarian_ids = {name: lid for lid, name in librarians}
    ...
    tk.Label(top_frame, text="Читальний зал:", bg="#f0f0f0", font=("Arial", 11)).pack(side=tk.LEFT, padx=5)
    room_menu = ttk.Combobox(top_frame, textvariable=room_var, state="readonly", width=20)
    room_menu['values'] = [''] + [name for _, name in rooms]
    room_menu.current(0)
    room_menu.pack(side=tk.LEFT, padx=5)

    tk.Label(top_frame, text="Бібліотекар:", bg="#f0f0f0", font=("Arial", 11)).pack(side=tk.LEFT, padx=5)
    librarian_menu = ttk.Combobox(top_frame, textvariable=librarian_var, state="readonly", width=20)
    librarian_menu['values'] = [''] + [name for _, name in librarians]
    librarian_menu.current(0)
    librarian_menu.pack(side=tk.LEFT, padx=5)

    tk.Label(top_frame, text="Категорія:", bg="#f0f0f0", font=("Arial", 11)).pack(side=tk.LEFT, padx=5)
    category_menu = ttk.Combobox(top_frame, textvariable=category_var, state="readonly", width=20)
    category_menu['values'] = ('', 'книги', 'журнали', 'газети', 'збірники статей',
                              'збірники віршів', 'дисертації', 'реферати', 'збірники доповідей і тез доповідей')
    category_menu.current(0)
    category_menu.pack(side=tk.LEFT, padx=5)

    tk.Label(top_frame, text="Назва:", bg="#f0f0f0", font=("Arial", 11)).pack(side=tk.LEFT, padx=5)
    search_entry = tk.Entry(top_frame, textvariable=search_var, width=25)
    search_entry.pack(side=tk.LEFT, padx=5)

    def refresh_books(*args):
        # Фільтруємо книги по вибраним фільтрам
        selected_room = room_var.get()
        room_id = room_ids.get(selected_room) if selected_room else None

        selected_category = category_var.get() if category_var.get() != '' else None
        title_search_val = search_var.get().strip() if search_var.get().strip() != '' else None

        show_books(reader_id, category=selected_category, title_search=title_search_val, room_filter=room_id)

    # Оновлення при зміні залу або категорії
    room_var.trace_add('write', refresh_books)
    category_var.trace_add('write', refresh_books)

    # Пошук по назві по кнопці
    search_button = tk.Button(top_frame, text="Пошук", command=refresh_books)
    search_button.pack(side=tk.LEFT, padx=5)

    # Бібліотекар не фільтрує книги, тому не прив’язуємо оновлення

    # Фрейми для книжок та повернень
    ...
    # Команди взяття книги тепер отримують room_id і librarian_id з Combobox

    def make_read_in_room_command(bid):
        def command():
            if not room_var.get():
                messagebox.showwarning("Увага", "Будь ласка, оберіть читальний зал.")
                return
            if not librarian_var.get():
                messagebox.showwarning("Увага", "Будь ласка, оберіть бібліотекаря.")
                return
            room_id = room_ids[room_var.get()]
            librarian_id = librarian_ids[librarian_var.get()]
            # Перевірка кількості доступних та запис
            conn = get_db_connection()
            cur = conn.cursor()
            cur.execute("""
                SELECT quantity - (
    ...
    # Оновлена функція show_books з підтримкою нових кнопок
    def show_books(reader_id, category=None, title_search=None, room_filter=None):
        for widget in available_books_frame.winfo_children():
            widget.destroy()

        # Тут твій оригінальний SQL з параметрами category, title_search, room_filter
        conn = get_db_connection()
        cursor = conn.cursor()
        query = """
        ...
        for book_id, title, book_type, access_type, is_in_room, is_issued_by_user, available_quantity in books:
            row = tk.Frame(available_books_frame, bg="#e6e6e6" if is_issued_by_user else "#ffffff")
            row.pack(fill=tk.X, padx=10, pady=3)

            tk.Label(row, text=title, font=("Arial", 11), width=30, anchor="w", bg=row.cget("bg")).pack(side=tk.LEFT)
            tk.Label(row, text=book_type, font=("Arial", 11), width=15, anchor="w", bg=row.cget("bg")).pack(side=tk.LEFT)
            tk.Label(row, text=is_in_room, font=("Arial", 11), width=20, anchor="w", bg=row.cget("bg")).pack(side=tk.LEFT)
            tk.Label(row, text=str(available_quantity), font=("Arial", 11), width=15, anchor="w", bg=row.cget("bg")).pack(side=tk.LEFT)
    ...
    # Показати книги спочатку без фільтрів
    show_books(reader_id)
    show_returned_books_inline(reader_id, return_books_frame)
