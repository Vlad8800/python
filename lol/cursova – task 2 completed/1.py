...
def show_books(reader_id, category=None, title_search=None, room_filter=None, get_ids=None):
    ...
    SELECT 
        ...
        EXISTS (
            ...
            WHERE ib.book_id = b.book_id
              AND ib.reader_id = %s
              AND (
                  (ib.reading_place = 'У читальній залі і в дома' 
                   AND (ib.return_date IS NULL OR ib.return_date >= CURDATE()))
                  OR 
                  (ib.reading_place = 'Тільки в читальній залі' 
                   AND ib.issue_date = CURDATE())
              )
        ...
        b.quantity - (
            ...
            WHERE ib2.book_id = b.book_id
              AND (
                  (ib2.reading_place = 'У читальній залі і в дома' 
                   AND (ib2.return_date IS NULL OR ib2.return_date >= CURDATE()))
                  OR 
                  (ib2.reading_place = 'Тільки в читальній залі' 
                   AND ib2.issue_date = CURDATE())
              )
    ...
    for book_id, title, category_name, access_type, is_in_room, is_issued_by_user, available_quantity in books:
        ...
        can_read_in_room = is_in_room == 'Так' and access_type in ['Тільки в читальній залі', 'У читальній залі і вдома']
...
def show_reader_window(reader_id):
    ...
    rooms = fetch_reading_rooms()            # [(room_id, name), ...]
    room_ids = {name: rid for rid, name in rooms}

    librarians = fetch_librarians()          # [(librarian_id, name), ...]
...
def show_admin_window(user_login):
    ...
    access_combo = Combobox(admin_window, values=["Тільки в читальній залі", "У читальній залі і вдома"], state="readonly", width=35)
...
def show_librarian_stats():
    ...
    librarian_combo = Combobox(selection_frame, values=list(librarian_map.keys()), state="readonly", width=30)
